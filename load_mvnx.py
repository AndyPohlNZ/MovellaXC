...
FOOT_CONTACT_MAP = {
    "LeftFoot_Heel": {
        "type": 1,
        "segment_index": mvn.SEGMENT_LEFT_FOOT,
        "point_index": 2,
    },
    "LeftFoot_Toe": {
        "type": 1,
        "segment_index": mvn.SEGMENT_LEFT_TOE,
        "point_index": 1,
    },
    "RightFoot_Heel": {
        "type": 1,
        "segment_index": mvn.SEGMENT_RIGHT_FOOT,
        "point_index": 2,
    },
    "RightFoot_Toe": {
        "type": 1,
        "segment_index": mvn.SEGMENT_RIGHT_TOE,
        "point_index": 1,
    },
}
...
def load_mvnx(file_name):
    ...
    mvnx_file.file_data["meta_data"]["original_filename"] = subject_element.get(
        "originalFilename"
    )
    mvnx_file.file_data["meta_data"]["configuration"] = subject_element.get(
        "configuration"
    )
    mvnx_file.file_data["meta_data"]["scenario"] = subject_element.get("userScenario")
    mvnx_file.file_data["meta_data"]["quality"] = subject_element.get(
        "processingQuality"
    )
    ...
    if sensors_element is not None:
        mvnx_file.file_data["sensors"] = parse_sensor(
            sensors_element, mvnx_file.file_data["segments"]["names"]
        )

    # Parse joint information
    joints_element = subject_element.find("mvn:joints", ns)
    if joints_element is not None:
        mvnx_file.file_data["joints"] = parse_joints(
            joints_element, mvnx_file.file_data["segments"]
        )
    ...
    # Parse foot contact
    foot_contact_definitions_element = subject_element.find(
        "mvn:footContactDefinition", ns
    )
    if foot_contact_definitions_element is not None:
        foot_contact_definition_elements = foot_contact_definitions_element.findall(
            "mvn:contactDefinition", ns
        )
        for foot_contact_definition_element in foot_contact_definition_elements:
            ...
            mvnx_file.file_data["foot_contact_def"][contact_index] = FOOT_CONTACT_MAP[
                contact_label
            ]

    # Parse the finger segments and their points
    for side in mvnx_file.file_data["finger_segments"]["elements"]:
        finger_segment_elements = subject_element.find(
            "mvn:fingerTrackingSegments" + side.capitalize(), ns
        )
        if finger_segment_elements is not None:
            finger_segments = parse_segments(finger_segment_elements)
            mvnx_file.file_data["finger_segments"]["elements"][side] = finger_segments[
                "elements"
            ]
            mvnx_file.file_data["finger_segments"]["names"][side] = finger_segments[
                "names"
            ]

    # Parse finger joint information
    for side in mvnx_file.file_data["finger_joints"]["elements"]:
        finger_joints_element = subject_element.find(
            "mvn:fingerTrackingJoints" + side.capitalize(), ns
        )
        if finger_joints_element is not None:
            finger_segments = {
                "names": mvnx_file.file_data["finger_segments"]["names"][side],
            ...
            mvnx_file.file_data["finger_joints"]["elements"][side] = finger_joints[
                "elements"
            ]
    ...
    (
        mvnx_file.file_data["frames"],
        mvnx_file.file_data["tpose"],
        mvnx_file.file_data["tpose_isb"],
        mvnx_file.file_data["identity"],
    ) = parse_frames(frames_element, mvnx_file)
...
def parse_joints(joints_element, segments):
    ...
    for joint_element in joint_elements:
        ...
        segment1_index, point1_index = get_connector_indices(
            joint_element, "mvn:connector1", segments
        )
        segment2_index, point2_index = get_connector_indices(
            joint_element, "mvn:connector2", segments
        )
        joint["seg_points"] = np.array(
            [[segment1_index, point1_index], [segment2_index, point2_index]]
        )
        joints.append(joint)
...
def parse_ergo_joints(ergo_joints_element):
    ...
    for ergo_joint_index in range(len(ergo_joint_elements)):
        ...
        ergo_joint = {
            "label": ergo_joint_element.get("label"),
...
def parse_segments(segment_elements):
    ...
    for segment_index in range(len(segment_elements)):
        ...
        segment = {
            "points_mvn": collections.OrderedDict(),
            "type": "Segment",
            "info": {},
            "label": segment_name,
        }
        ...
        for point_index in range(len(point_elements)):
            ...
            segment["points_mvn"][point_name] = np.array(
                [float(pos) for pos in pos_b.text.split(" ")]
            )

            # if the point offset is really small, consider it the origin (could just pick 1st or all 0's as well)
            if (
                np.sqrt(np.sum([sq * sq for sq in segment["points_mvn"][point_name]]))
                < 0.00001
            ):
...
def get_connector_indices(joint_element, connector, segments):
    ...
    point_index = -1
    for key, value in segments["elements"][tokens[0]]["info"][
        "point_label_from_index"
    ].items():
        if value == tokens[1]:
            point_index = key
...
def parse_frames(frames_element, mvnx_file):
    ...
    for frame_element in tqdm(frame_elements):
        if frame_element.get("type") == "normal":
            ...
            frames["joint_data"].append(
                get_joint_data_from_frame(
                    frame_element, "jointAngle", mvnx_file.file_data["joints"]["names"]
                )
            )
            frames["joint_data_xzy"].append(
                get_joint_data_from_frame(
                    frame_element,
                    "jointAngleXZY",
                    mvnx_file.file_data["joints"]["names"],
                )
            )
            frames["ergo_joint_data"].append(
                get_joint_data_from_frame(
                    frame_element,
                    "jointAngleErgo",
                    mvnx_file.file_data["ergo_joints"]["names"],
                )
            )
            ...
            frames["segment_data"].append(
                get_segment_data_from_frame(
                    frame_element, mvnx_file.file_data["segments"]["names"]
                )
            )
            frames["sensor_data"].append(
                get_sensor_data_from_frame(
                    frame_element, mvnx_file.file_data["sensors"]["names"]
                )
            )
            frames["contacts_data"].append(
                get_contact_data_from_frame(
                    frame_element, mvnx_file.file_data["foot_contact_def"]
                )
            )
            frames["finger_segment_data"].append(
                get_finger_data_from_frame(
                    frame_element, mvnx_file.file_data["finger_segments"]["names"]
                )
            )
            for side in frames["finger_joint_data"]:
                element_name = "jointAngleFingers" + side.capitalize()
                frames["finger_joint_data"][side].append(
                    get_joint_data_from_frame(
                        frame_element,
                        element_name,
                        mvnx_file.file_data["finger_joints"]["names"][side],
                    )
                )
                element_name = "jointAngleFingers" + side.capitalize() + "XZY"
                frames["finger_joint_data_xzy"][side].append(
                    get_joint_data_from_frame(
                        frame_element,
                        element_name,
                        mvnx_file.file_data["finger_joints"]["names"][side],
            ...
            tpose = get_t_pose_data_from_frame(
                frame_element, mvnx_file.file_data["segments"]["names"]
            )
        elif frame_element.get("type") == "tpose-isb":
            tpose_isb = get_t_pose_data_from_frame(
                frame_element, mvnx_file.file_data["segments"]["names"]
            )
        elif frame_element.get("type") == "identity":
            identity = get_t_pose_data_from_frame(
                frame_element, mvnx_file.file_data["segments"]["names"]
            )
...
def get_t_pose_data_from_frame(frame_element, segment_names):
    ...
    for index in range(len(segment_names)):
        segment = {
            "pos_g": get_3d_vector(offsets, index),
...
def get_segment_data_from_frame(frame_element, segment_names):
    ...
    for index in range(len(segment_names)):
        ...
        segment_data[segment_name]["ang_acc"] = get_3d_vector(
...
def get_sensor_data_from_frame(frame_element, sensor_names):
    ...
    free_accelerations = frame_element_as_floats(
        frame_element, "sensorFreeAcceleration"
    )
...
def get_finger_data_from_frame(frame_element, finger_segment_names):
    ...
    for side in finger_data:
        orientations = frame_element_as_floats(
            frame_element, "orientationFingers" + side.capitalize()
        )
        offsets = frame_element_as_floats(
            frame_element, "positionFingers" + side.capitalize()
        )
...
def frame_element_as_floats(frame_element, element):
    ...
    return (
        [float(value) for value in element_value.text.split(" ")]
        if element_value is not None
        else []
    )
