...
class MvnxFileAccessor:
    ...
    def ergo_joint_count(self):
        ergo_joint_count = (
            len(self.file_data["ergo_joints"])
            if self.file_data["ergo_joints"] is not None
            else None
        )
    ...
    def window_profile(self):
        ...
        if "profiles" not in self.file_data["meta_data"]:
            ...
            for profile_spec in self.file_data["meta_data"]["profiles"]:
                if (
                    profile_spec[1] > self._first_frame
                    and profile_spec[0] <= self._last_frame
                ):
    ...
    def frame_to_mapped_slice(self, frame) -> (slice, bool):
        ...
        if not isinstance(frame, slice):
            if frame == mvn.FRAMES_ALL:
                ...
                frame = slice(
                    self._first_frame + frame, self._first_frame + frame + 1
                )  # create a single frame slice
                single_frame = True
        else:
    ...
    def t_pose_is_valid(self):
        return ("tpose" in self.file_data) and (self.t_pose()["segments_counts"] > 0)
    ...
    def get_segment_data(
        self, data_field, segment, frame=mvn.FRAMES_ALL, axis=mvn.AXIS_ALL
    ):
    ...
    def get_joint_angle(self, joint, frame=mvn.FRAMES_ALL, angle=mvn.ANGLE_ALL):
        ...
        if angle == mvn.ANGLE_ALL:
            return_values = [
                value[joint_name] for value in self.file_data["frames"][data_set][frame]
            ]
        else:
            return_values = [
                value[joint_name][angle]
    ...
    def get_sensor_data(
        self, data_field, sensor_segment, frame=mvn.FRAMES_ALL, axis=mvn.AXIS_ALL
    ):
    ...
    def has_foot_contact(self, frame, foot_contact_flags=0):
        ...
        """
        frame_contacts = self.get_foot_contacts(
            frame
        )  # frame will be shifted in the called method
        ...
        if isinstance(frame_contacts, int):
            ...
            for contacts in frame_contacts:
                if foot_contact_flags == 0:
                    ...
                    has_contacts.append(
                        True if (contacts & foot_contact_flags) > 0 else False
                    )
            return has_contacts
    ...
    def get_data(
        ...
        if data_set == "joint_data":
            ...
            return_values = [
                value[segment_name][data_field]
                for value in self.file_data["frames"][data_set][frame]
            ]
        else:
            return_values = [
                value[segment_name][data_field][axis]
                for value in self.file_data["frames"][data_set][frame]
            ]
